...
SOURCES 	= main.c \
	...
	SplitTokenList.c \
	FloatList.c \
	BlackBoxActionList.c \
	HeuristicMeasure.c \
	HeuristicMeasureList.c \
	InterfaceGraph.c \
	Splitability.c \
	Groundability.c \
	SplitMain.c \
...
ff: $(OBJECTS) $(PDDL_PARSER_OBJ)
	$(CC) -o splitting $(OBJECTS) $(PDDL_PARSER_OBJ) $(CFLAGS) $(LIBS)
...
veryclean: clean
	rm -f ff H* J* K* L* O* TIME* graph.* SEARCHTIME* *.symbex gmon.out \
	$(PDDL_PARSER_SRC) \
	lex.fct_pddl.c lex.ops_pddl.c lex.probname.c \
	*.output domain.pddl facts.pddl
