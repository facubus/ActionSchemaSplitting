...

{
  ...
  if ( gcmd_line.display_info == 103 ) {
    ...
    for ( i = 0; i < gnum_types; i++ ) {
      printf("\n%d --> %s (universal? %d): ", 
	     i, gtype_names[i], guniversal_type[i]);
      for ( j = 0; j < gtype_size[i]; j++ ) {
	printf("(%d) %d ", 
	       gmember_nr[gtype_consts[i][j]][i], gtype_consts[i][j]);
      }
    }
...
void check_universal_types( void )
...

{
  ...
  for ( i = 0; i < lnum_types; i++ ) {
    ...
    gtype_names[gnum_types] = new_Token( strlen( ltype_names[i] ) + 1 );
    strcpy( gtype_names[gnum_types], ltype_names[i] );
  ...
  for ( f = gorig_constant_list; f; f = f->next ) {
	...
	exit( 1 );
      ...
      if ( ocount >= gnum_consts - gnum_domain_consts ) {
	gdomain_const[gnum_constants] = TRUE;
      } else {
	gdomain_const[gnum_constants] = FALSE;
      }
